...
void main() {
    uv = rotate_cw(a_pos, PI * .25) * 3 * u_size * u_size;
    vec2 world_pos = u_pos + u_box * uv;
...
void main() {
    ...
    color = alpha_blend(color, mix(vec4(0), u_color, float(abs(square_uv.x) + abs(square_uv.y) < 4 * u_size)));
    vec2 icon_uv = uv * .6 * (1.2 / u_size / u_size) + .5;
