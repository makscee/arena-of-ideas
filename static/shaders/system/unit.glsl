...
void main() {
    card = get_card_value();
    uv = a_pos * (1.0 + u_padding + vec2(0.3, 2.8) * card);
    vec2 pos = get_card_uv(uv * u_radius, card) + u_position;
...
const vec2 CARD_OFFSET = vec2(0, 0.8);
...
vec4 draw_card(vec4 unit_color, vec2 unit_uv) {
    vec2 uv = uv * .5 + CARD_OFFSET;
    float card_sdf = rectangle_sdf(uv, CARD_SIZE * mix(3, 1, card), 0);
    ...
    return mix(unit_color, color, card);
}
