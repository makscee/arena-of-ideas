...
uniform float u_p_vel_circle = 0;
uniform float u_p_vel_circle_mul = 1;
uniform float u_p_vel_circle_shift = 0;
...
void main() {
    ...
    vec2 gravity = vec2(0, -1) * u_p_vel_gravity * box.x;
    vec2 center = -start_pos * u_p_vel_center;
    float circle_pos = p_t * u_p_vel_circle_mul + u_p_vel_circle_shift * r3;
    vec2 circle = vec2(cos(circle_pos), sin(circle_pos)) * u_p_vel_circle * box.x;
    vec2 velocity = gravity + center + circle;
