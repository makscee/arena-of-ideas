// AOI-71: one authenticated desktop season through released governance, the
...
import { BASE, armGuard, check, finish, launch, loginViaUi } from "./lib.mjs";

const PLAYTEST_DESKTOP = { width: 1440, height: 1000 };
...
const out = process.env.SHOTS_DIR ?? join(process.cwd(), "e2e/.evidence/player-driven-desktop-v1-playtest");
...
function context(ip) {
  return browser.newContext({ viewport: PLAYTEST_DESKTOP, extraHTTPHeaders: { "x-forwarded-for": ip } });
}
...
async function capture(page, file, boundary) {
  const width = await page.evaluate(() => ({ scroll: document.documentElement.scrollWidth, client: document.documentElement.clientWidth }));
  check(width.scroll === width.client, `${file} has no horizontal page overflow`, `${width.scroll}/${width.client}`);
  const path = join(out, file);
  await page.screenshot({ path, fullPage: false });
  const png = readFileSync(path);
  const dimensions = { width: png.readUInt32BE(16), height: png.readUInt32BE(20) };
  check(
    dimensions.width === PLAYTEST_DESKTOP.width && dimensions.height === PLAYTEST_DESKTOP.height,
    `${file} is exactly ${PLAYTEST_DESKTOP.width}x${PLAYTEST_DESKTOP.height}`,
...
check((await maks.locator("#hub-season").textContent()).includes("Season 1 · content v1"), "player orients on the season-1 hub");
await capture(maks, "still-00-login-profile-hub.png", "Visible login/profile completes at the desktop hub with Maks and Season 1/content v1.");
...
const playerVoteRow = rowByText(maks, glassText);
await playerVoteRow.locator(".ideas-vote-down").click();
await maks.waitForFunction((text) => {
  const row = [...document.querySelectorAll("#ideas-list .ideas-row")].find((node) => node.querySelector(".ideas-text")?.textContent === text);
  return row?.querySelector('.ideas-vote-down[aria-pressed="true"]') !== null;
}, glassText);
check((await playerVoteRow.locator(".ideas-vote-down").getAttribute("aria-pressed")) === "true", "Maks casts a visible directional down-vote");
await capture(maks, "still-01-browser-submit-directional-vote.png", "Maks's browser-authored Frostbite idea and visible directional down-vote on the glass-cannon submission coexist in season 1.");
...
check(bouncedText.includes("bounced") && bouncedText.includes(bounceReason) && bouncedText.includes("4/6 yes"), "glass branch shows exact reason and retained votes", bouncedText);
await capture(maks, "still-04-ship-bounce-season2.png", "Season 2/content v2 shows shipped Frostbite and bounced glass with exact reason and retained 4/6 tally, including the player's down-vote.");
...
await maks.click('[data-fusion-first="0"]');
const reversePreview = await maks.locator('[data-fuse="0:1"]').textContent();
check(reversePreview.includes("Venomancer + Frostbiter"), "reverse ordered preview names Venomancer first and Frostbiter second", reversePreview);
await capture(maks, "still-12a-reverse-ordered-fusion-preview.png", "Supported A+B preview explicitly reads Venomancer + Frostbiter before comparison.");
await maks.click("[data-fusion-cancel]");
...
check(reversePreview !== preview, "A+B and B+A previews are visibly distinct", `${reversePreview} / ${preview}`);
await capture(maks, "still-12b-committed-order-fusion-preview.png", "Supported B+A preview explicitly reads Frostbiter + Venomancer after comparison.");
await capture(maks, "motion-02-ordered-preview.png", "Frame walk: both orders compared; chosen legal ordered fusion preview before commit.");
...
const continuity = {
  scenario: "player-driven-desktop-v1-playtest",
  ...
  governance: {
    ...
    bounced: { ideaId: "idea-1", reason: persisted.bouncedIdea.bounce_reason, retainedTally: "4/6 yes", playerDirection: "down" },
...
await browser.close();
disarm();
finish("player-driven-desktop-v1-playtest");
