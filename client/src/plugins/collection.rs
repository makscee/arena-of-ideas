...
fn collection_ui(
    ...
    egui::TopBottomPanel::top("top_bar").show(ctx, |ui| {
        ui.horizontal(|ui| {
            ...
            ui.separator();
            if ui.selectable_label(state.tab == CollectionTab::Abilities, "Abilities").clicked() {
    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        for (i, ability) in content.abilities.iter().enumerate() {
            ...
            let response = ui.group(|ui| {
                ui.horizontal(|ui| {
                    ui.colored_label(
                        crate::plugins::ui::colors::ABILITY_COLOR,
                        &ability.name,
                    );
                    ui.label("→");
                    ui.label(&ability.target_type);
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ui.colored_label(rating_color(ability.rating), format!("{:+}", ability.rating));
    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        for (i, unit) in content.units.iter().enumerate() {
            ...
            let response = ui.group(|ui| {
                ui.horizontal(|ui| {
                    ui.colored_label(
                        tier_color(unit.tier),
                        format!("T{}", unit.tier),
                    );
                    ui.colored_label(
                        crate::plugins::ui::colors::UNIT_COLOR,
                        &unit.name,
                    );
