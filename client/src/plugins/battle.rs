...
mod tests {
    ...
    fn test_shield_absorbs_damage() {
        ...
        assert!(
            result.turns > 5,
            "Shield should help guardian survive longer, got {} turns",
            result.turns
        );
    }
    ...
    fn battle_high_pwr_one_shots() {
        ...
        assert!(
            result.turns <= 2,
            "Should one-shot in 1-2 turns, took {}",
            result.turns
        );
    }
    ...
    fn battle_damage_taken_trigger_fires_on_hit() {
        ...
        assert!(
            counter_used,
            "DamageTaken trigger should fire Counter ability"
        );
    }
    ...
    fn battle_ally_death_trigger_fires() {
        ...
        assert!(
            avenge_used,
            "AllyDeath trigger should fire Avenge when ally dies"
        );
    }
