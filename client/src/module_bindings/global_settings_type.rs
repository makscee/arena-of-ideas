// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#![allow(unused, clippy::all)]
use spacetimedb_sdk::__codegen::{self as __sdk, __lib, __sats, __ws};

#[derive(__lib::ser::Serialize, __lib::de::Deserialize, Clone, PartialEq, Debug)]
#[sats(crate = __lib)]
pub struct GlobalSettings {
    pub always_zero: u32,
    pub starting_gold: i32,
    pub unit_sell_value: i32,
    pub reroll_cost: i32,
    pub gold_reward: i32,
    pub fatigue_start_turn: i32,
}

impl __sdk::InModule for GlobalSettings {
    type Module = super::RemoteModule;
}

/// Column accessor struct for the table `GlobalSettings`.
///
/// Provides typed access to columns for query building.
pub struct GlobalSettingsCols {
    pub always_zero: __sdk::__query_builder::Col<GlobalSettings, u32>,
    pub starting_gold: __sdk::__query_builder::Col<GlobalSettings, i32>,
    pub unit_sell_value: __sdk::__query_builder::Col<GlobalSettings, i32>,
    pub reroll_cost: __sdk::__query_builder::Col<GlobalSettings, i32>,
    pub gold_reward: __sdk::__query_builder::Col<GlobalSettings, i32>,
    pub fatigue_start_turn: __sdk::__query_builder::Col<GlobalSettings, i32>,
}

impl __sdk::__query_builder::HasCols for GlobalSettings {
    type Cols = GlobalSettingsCols;
    fn cols(table_name: &'static str) -> Self::Cols {
        GlobalSettingsCols {
            always_zero: __sdk::__query_builder::Col::new(table_name, "always_zero"),
            starting_gold: __sdk::__query_builder::Col::new(table_name, "starting_gold"),
            unit_sell_value: __sdk::__query_builder::Col::new(table_name, "unit_sell_value"),
            reroll_cost: __sdk::__query_builder::Col::new(table_name, "reroll_cost"),
            gold_reward: __sdk::__query_builder::Col::new(table_name, "gold_reward"),
            fatigue_start_turn: __sdk::__query_builder::Col::new(table_name, "fatigue_start_turn"),
        }
    }
}

/// Indexed column accessor struct for the table `GlobalSettings`.
///
/// Provides typed access to indexed columns for query building.
pub struct GlobalSettingsIxCols {
    pub always_zero: __sdk::__query_builder::IxCol<GlobalSettings, u32>,
}

impl __sdk::__query_builder::HasIxCols for GlobalSettings {
    type IxCols = GlobalSettingsIxCols;
    fn ix_cols(table_name: &'static str) -> Self::IxCols {
        GlobalSettingsIxCols {
            always_zero: __sdk::__query_builder::IxCol::new(table_name, "always_zero"),
        }
    }
}

impl __sdk::__query_builder::CanBeLookupTable for GlobalSettings {}
