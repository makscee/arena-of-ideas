...
- **Decision-sourced determinism [PINNED].** A run is `seed + decision sequence → RunState + run log`; every transition (initRun/buy/reroll/reorder/fight/ladderFight) returns a fresh RunState and never mutates its input. `ladderFight` does mutate its store — the run layer's one mutable boundary: it gains the ghost, and possibly a new champion. Battle seeds come off the run's one RNG stream, and the run log is byte-comparable JSONL like the battle log. A ladder run is reproducible from (seed, decisions, ladder contents). No wall-clock reads anywhere in the kernel; a timestamp, if wanted, comes in as data.
- **The ladder is built out of played runs [PINNED, v3 arc].** A Ladder holds, per round, a pool of team snapshots (ghosts) plus one champion spot, behind the `LadderStore` interface (in-memory, file, and browser-localStorage backings — the persistent ones share one serialized `LadderData` shape); the kernel logic depends only on the interface.
- **A RunState round-trips JSON by value** (`serializeRun`/`deserializeRun`), pool and statuses included — RunState holds no functions, so a stored run is self-contained and resumes deterministically; a corrupt store is refused loudly.
...
- **Opponent draw:** one seeded draw, uniform over the round-R pool minus the run's own ghosts — deterministic given the run's RNG state and pool contents. Every drawn team passes the content gate, like any battle input — and it passes *before* the run's own ghost persists, so a gate-failing opponent aborts the fight without growing the pool on every retry. (Own ghosts are excluded from the draw, so gating first changes nothing else.)
- **Champion rule:** an empty draw means the run has outrun every ghost at this round and challenges the champion. A win — or a vacant spot — crowns the fielded team: the run ends `crown`, and the spot persists across runs. A dethroned champion loses only the spot; its ghosts stay. A lost challenge is a normal loss. The vacant-spot crown is a kernel edge only: openLadder seats a champion, so it is unreachable on any opened ladder.
...
- **Bootstrap:** an empty ladder seeds rounds 1..BOOTSTRAP_DEPTH from the shipped bootstrap teams (tunables), escalating in strength, and seats BOOTSTRAP_CHAMPION — the strongest shipped team — in the champion spot, so a first-ever run has a real climb and a crown is always earned by beating someone (a fresh ladder crowned every run through the vacant spot, most with losing records). Every bootstrap team passes the content gate at open. A played-on ladder can never have an empty round-1 pool — snapshot-before-fight fills it.
