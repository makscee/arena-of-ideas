...
- **Bootstrap:** an empty ladder seeds rounds 1..BOOTSTRAP_DEPTH from the shipped bootstrap teams (tunables), escalating in strength, so a first-ever run has a real climb before the vacant spot. Every bootstrap team passes the content gate at open. A played-on ladder can never have an empty round-1 pool — snapshot-before-fight fills it.
