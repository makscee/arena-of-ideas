...
//
// This slice owns the shop economy only. fight() takes its opponent as a
// parameter and delegates to battle(); opponent selection and run-end at
// 0 lives belong to the ladder layer above. Invalid decisions are rejected
// loudly, the validate.ts manner — a silently ignored buy would desync the
// stored decision sequence from the state it claims to reproduce.
...
export type RunEventBody =
  | { type: "RunStart"; seed: number; gold: number; lives: number }
...
/** Start a run: starting gold and lives, round 1's shop rolled from the seed. */
export function initRun(input: RunInput): RunState {
  if (input.pool.length === 0) throw new Error("a run needs a non-empty draft pool");
  const s: RunState = {
    seed: input.seed,
    round: 1,
    gold: STARTING_GOLD,
  ...
  emit(s, { type: "RunStart", seed: input.seed, gold: s.gold, lives: s.lives });
  rollOffers(s);
  return s;
}
 ...
 * the round: a loss costs a life, income lands on the carryover, the shop rerolls
 * free. Delegates to battle(); only the outcome enters the run log — the full
 * battle log is reproducible from the logged battleSeed and the teams. */
export function fight(state: RunState, opponent: UnitDef[]): RunState {
  ...
  const s = clone(state);
  // The battle seed comes off the run's own stream: one seed drives the whole run.
  ...
  if (end.winner === "B") s.lives -= 1; // a draw costs no life; run-end at 0 lives is slice 2's rule
