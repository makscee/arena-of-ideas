...
impl<'w> Context<'w> {
    ...
    pub fn from_world_ref_r(
        ...
        let cs = ContextSource::WorldRef(world);
        let mut context = Context {
            sources: [cs].into(),
            ..default()
        };
    ...
    pub fn add_owner(&mut self, owner: Entity) {
        self.layers.push(ContextLayer::Owner(owner));
    }
    pub fn add_target(&mut self, target: Entity) {
    ...
    pub fn set_value_var(&mut self, value: VarValue) {
