...
export function validatePool(units: unknown, statuses: StatusRegistry, abilities: AbilityRegistry, label = "pool"): ValidationIssue[] {
  ...
  units.forEach((u, i) => {
    ...
    if (isObject(u)) {
      const refs = u["abilities"];
      if (Array.isArray(refs) && refs.length !== 1) {
        issues.push({
          path: `${label}[${i}].abilities`,
          message: `a draft-pool base Unit must carry exactly one Ability; got ${refs.length} — two-Ability Units are generated by fusion, never drafted`,
        });
      }
    }
